...
const int NUM_CH = 1; //number of channnels
...
void loop() {
  byte data[4]; //data array, 3byte read value and 1byte status register
  ...
  while (i<NUM_CH){
    ...
    t1 = micros();
    readvalue = 0;
    readvalue += (long)data[0] << 24;
    readvalue += (long)data[1] << 16;
    readvalue += (long)data[2] << 8;
    readvalue += (long)data[3];
    Serial.println(readvalue,BIN);
    
    //Serial.println(t1-t0,5);
    if  (i == NUM_CH){
      break;
    }
    //voltage = ConvertVoltage(readvalue);
