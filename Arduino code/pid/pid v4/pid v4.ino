...
float Kp=600;
float Ti=6000.0; //Integrator time constant 
...
void loop() {
  ...
  while (i<6){
    ...
    if (integral_enable){
      outbuf=output[i]-Kp*(delta_err[i]+ deltat/1000000.0/Ti*err[i] + Td/deltat/1000000.0*(delta_err[i]-prev_delta_err[i]));
      output[i] = int(constrain(outbuf,0,outmax)); // heat only
    }
    else{
      outbuf=-Kp*(err[i]);
      output[i] = int(constrain(outbuf,0,outmax)); // heat only
    ...
    prev_delta_err[i] = delta_err[i];
    prev_error[i]=err[i]; //previous err
