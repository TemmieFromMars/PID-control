...
void loop() {
  ...
  float outbuf;
  err[0] = T_set-(Tshield[0]+Tshield[1]+Tshield[2]+Tshield[3]+Tshield[4]+Tshield[5])/6;
  if (prev_error[0]-err[0] > 1 || prev_error[0]-err[0]<-1){
    if (pidflag == 0){
  ...
  delta_err[0] = err[0] - prev_error[0];
  if (integral_enable){
    outbuf=output[0]+Kp*(delta_err[0]+ deltat/1000000.0/Ti*err[0] + Td/deltat/1000000.0*(delta_err[0]-prev_delta_err[0]));
  ...
  prev_delta_err[0] = delta_err[0];
  prev_error[0]=err[0]; //previous err
