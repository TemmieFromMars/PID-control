...
void loop() {
  ...
  while (i<6){
    err = Tshield[i]-T_set;
    if (prev_error[i]-err > 0.2 || prev_error[i]-err<-0.2){
      //avoid any error in reading can affect control
      err = prev_error[i];
    }
    dtostrf(Tshield[i],7,5, TreadingBuf[i]);
    if (integral_enable){
      err_integral[i]=max(-outmax,min(err_integral[i]+err*deltat/1000000.0,outmax));
    }
    else{
      err_integral[i]=0; //integral of each face
    }
    
    output[i] = int(min(max(-Kp*(err+ err_integral[i]/Ti + Td*(err-prev_error[i])),0),outmax)); // heat only
    prev_error[i]=err; //previous err
    //analogWrite(PWMpin[4],1);
    if (PIDenable ==1){
      analogWrite(PWMpin[i],output[i]);//range 0-255, output to PWMpin
      if(i==5){
        analogWrite(PWMpin[i],min(255,4*output[i]));
      }
    }
    i+=1;
  }
