...
float T_set = 24.5; //set Temperature in C degree
//PID Parameters
//Kp (err+ Ingetal(err)/Ti + Td/Derivative(err))
float Kp=60;
float Ti=60.0; //Integrator time constant (seconds) 35
float Td=10; //Derivative time constant (seconds) 23
...
int i_cycle = 0;
float tstart=0; //timestamp when begin reading
float tend=0; //timestamp when finish reading
float deltat=0; //time used
/*
...
void loop() {
  ...
  deltat = (micros()-tstart)/1000000.0; // to seconds
  ...
  while (i<1){
    //Serial.println(Tmean1[0],7);
    err = Tmean1[0]-T_set;

    if (integral_enable){
      err_integral[i]=min(err_integral[i]+err*deltat,outmax);
    }
    else{
      err_integral[i]=0; //integral of each face
    }
    
    output[0] = int(min(max(-Kp*(err+ err_integral[i]/Ti + Td*(err-prev_error[i])),0),outmax)); // heat only
    //Serial.print("output:");
    //Serial.println(output[0]);
    prev_error[i]=err; //previous err
    //analogWrite(PWMpin[1],output[0]);
    //analogWrite(PWMpin[i],200);//range 0-255, output to PWMpin
    i+=1;
  }
