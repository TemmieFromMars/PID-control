...
float T_set = 23.2; //set Temperature in C degree
//PID Parameters
//Kp (err+ Ingetal(err)/Ti + Td/Derivative(err))
float Kp=800;
float Ti=200; //Integrator time constant 
...
const int N_loop = 12; // loops per serial_output
int i_loop = 0;
...
float prev_error[7]{0}; //previous err
float err_integral[6]={-3.2,0,0,0,0,0}; //integral -3.2 for 23.2 Tset and 4V Mosfet input
int output[6]{0}; //output to PWM to control range 0-255
float integralbuf[6]{0};
...
void loop() {
  ...
  //write buffer
  char strBuf[300];
  char TreadingBuf[25][10];
  i=0;
  while (i<NUM_CH){
    //Serial.println(i+1); 
    Tmean1[i] = Tread1[i]/cycles1[i];
    dtostrf(Tmean1[i],7,5, TreadingBuf[i]);
    Tmean2[i] = Tread2[i]/cycles2[i];
    dtostrf(Tmean2[i],7,5, TreadingBuf[i+12]);
    i+=1;
  }
  ...
  err = (Tmean1[0]+Tmean1[1]+Tmean2[1]+Tmean1[2]+Tmean1[3]+Tmean2[2]+Tmean2[3]+Tmean1[4]+Tmean1[5]+Tmean2[4]+Tmean2[5]+Tmean1[6]+Tmean1[7]+Tmean2[6]+Tmean2[7]+Tmean1[8]+Tmean1[9]+Tmean2[8]+Tmean2[9]+Tmean1[10]+Tmean1[11]+Tmean2[10]+Tmean2[11])/23.0-T_set;
  if(isnan(err)){
     // it's a NaN, use previous step
     err = prev_error[6];
  }
  if (integral_enable){
      outbuf = err_integral[0]+err*deltat/1000000.0;
      err_integral[0]=constrain(outbuf,-outmax,outmax);
      dtostrf(err_integral[0],7,5, TreadingBuf[24]);
  }
  else{
    err_integral[0]=0; //integral of each face
    dtostrf(err_integral[0],7,5, TreadingBuf[24]);
  }
  outbuf = -Kp*(0.45*err+ 0.54*err_integral[0]/Ti);
  output[0] = int(constrain(outbuf,0,outmax)); // heat only
  ...
  if (PIDenable ==1){
    while (i<6){
      ...
      if(i==5){
        analogWrite(PWMpin[i],min(255,3*output[0]));
      }
      else{
        analogWrite(PWMpin[i],output[0]);//range 0-255, output to PWMpin
      }
      i+=1;
    }
  ...
  if (i_loop>N_loop){
    sprintf(strBuf, "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%d",
    TreadingBuf[0],TreadingBuf[1],"0",TreadingBuf[13],
    TreadingBuf[2],TreadingBuf[3],TreadingBuf[14],TreadingBuf[15],
    TreadingBuf[4],TreadingBuf[5],TreadingBuf[16],TreadingBuf[17],
    TreadingBuf[6],TreadingBuf[7],TreadingBuf[18],TreadingBuf[19],
    TreadingBuf[8],TreadingBuf[9],TreadingBuf[20],TreadingBuf[21],
    TreadingBuf[10],TreadingBuf[11],TreadingBuf[22],TreadingBuf[23],
    TreadingBuf[24],output[0]);
