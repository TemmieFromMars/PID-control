...
float T_set = 23.3; //set Temperature in C degree
//PID Parameters
...
void loop() {
  ...
  while (i<NUM_CH){
    ...
    //Serial.println(Tmean1[i],4);
    Tmean2[i] = Tread2[i]/cycles2[i];
    //Serial.println(Tmean2[i],4);
    i+=1;
  }
  ...
  while (i<6){
    err = Tshield[i]-T_set;
    if (prev_error[i]-err > 1 || prev_error[i]-err<-1){
      //avoid any error in reading can affect control
      err = prev_error[i];
    }
    dtostrf(Tshield[i],7,5, TreadingBuf[i]);
    prev_error[i]=err;
    i+=1;
  }
  ...
  outbuf = -Kp*(err+ err_integral[0]/Ti + Td*(err-prev_error[6]));
  output[0] = int(constrain(outbuf,0,outmax)); // heat only
     //previous err
    //analogWrite(PWMpin[4],1);
  prev_error[6]=err;
  if (PIDenable ==1){
    while (i<6){
  ...
  sprintf(strBuf, "%s,%d,%s,%d,%s,%d,%s,%d,%s,%d,%s,%d",
   TreadingBuf[0],output[0], TreadingBuf[1],output[0], TreadingBuf[2],output[0], TreadingBuf[3],output[0], TreadingBuf[4],output[0], TreadingBuf[5],output[0]);
